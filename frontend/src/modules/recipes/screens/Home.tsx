...
const tutorialSteps: TutorialStep[] = [
    ...
    },
    {
        ...
        highlight: "Use texto ou voz para encontrar receitas rapidamente.",
        description:
            "Na home, a barra de busca fica acima dos filtros. Quando você digita algo, o feed some e aparecem só os resultados. Use o microfone para busca por voz.",
...
export default function Home({ navigation }: { navigation: any }) {
    ...
    const completeTutorial = useCallback(async () => {
        if (user?.id) await markAppTutorialAsSeen(user.id);
        ...
        useCallback(() => {
            ...
            async function loadRecipes() {
                try {
                    ...
                    const [favoritesResult, recipesResult] = await Promise.allSettled([
                        listFavoriteRecipes(),
                        getSuggestedRecipes(25),
                    ]);
    ...
    useEffect(() => {
        ...
        async function loadRecipesByExpiringIngredients() {
            try {
                ...
                if (isActive) setExpiringIngredientsLoading(false);
            }
        }
                        ...
                        <View className="flex flex-col gap-6">
                            {activeTab === "all" && (
                                <View className="flex flex-col">
                                    ...
                                    {errorMessage ? (
                                        <InlineError message={errorMessage} title="Não conseguimos carregar o feed" />
                                    ) : null}
                            ...
                            {activeTab === "pantry" && (
                                <View className="flex flex-col">
                                    <Text className="text-2xl font-bold mb-4">
                                        Receitas com ingredientes da sua despensa vencendo esta semana
                                    </Text>
