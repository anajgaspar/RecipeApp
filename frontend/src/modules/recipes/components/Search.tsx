...
export default function HomeSearch({ navigation, inputRef, onSearchingChange }: HomeSearchProps) {
    ...
    const isSearching = trimmedQuery.length > 0 || difficulty !== "" || category !== "";
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            void handleSearchRef.current("text");
        }, 300);

        return () => clearTimeout(timeout);
    }, [trimmedQuery, difficulty, category, isSearching]);
                    ...
                    speechModule.addListener(
                        ...
                        (event: { error: string; message?: string }) => {
                            ...
                            if (event.error !== "aborted") {
                                Alert.alert(
                                    "Busca por voz",
                                    event.message || "Não foi possível reconhecer sua fala.",
                                );
                            }
                        },
                    ),
        ...
        try {
            ...
            if (!permission.granted) {
                Alert.alert(
                    ...
                    "Permita o uso do microfone e do reconhecimento de fala para buscar por voz.",
                );
                return;
            }
            ...
            const message =
                error instanceof Error ? error.message : "Não foi possível iniciar a busca por voz.";
        ...
        <View className="gap-4">
            <View className="flex flex-row items-center gap-3">
                ...
                >
                    <Ionicons
                        ...
                        size={20}
                        color={isListening ? "#f97316" : "black"}
                    />
                </Pressable>
                <Pressable
                    onPress={() => setShowFilters((v) => !v)}
                    className={`rounded-full p-3 border ${showFilters ? "border-[#f97316] bg-[#f97316]/10" : "border-[#9ca3af]"}`}
                >
                    <Ionicons
                        name="options-outline"
                        size={20}
                        color={showFilters ? "#f97316" : "black"}
                    />
                </Pressable>
            </View>
            {showFilters ? (
            ...
            {isSearching ? (
                <View className="flex flex-col">
                    ...
                    {errorMessage ? (
                        <InlineError message={errorMessage} title="Falha ao buscar receitas" />
                    ) : null}
