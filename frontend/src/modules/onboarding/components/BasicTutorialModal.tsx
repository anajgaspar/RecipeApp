        ...
        <View className="flex-1 justify-between">
          <View className="flex-row items-center justify-between pt-8">
            <View>
              ...
              <Text className="text-white/70 text-sm">Guia básico para começar sem perder tempo.</Text>
            </View>
          ...
          <View className="rounded-[28px] border border-white/10 bg-[#111827]/92 p-5 shadow-2xl">
            <View className="mb-4 flex-row items-center gap-3">
              <View className="h-12 w-12 items-center justify-center rounded-2xl bg-[#f97316]">
                <FontAwesome6 name="sparkles" size={18} color="#ffffff" />
              </View>
            ...
            {step.actionLabel ? (
              <Pressable
                onPress={onAction}
                className="mt-4 rounded-2xl border border-[#f97316]/30 bg-[#f97316]/10 px-4 py-3"
              >
                <Text className="text-[#fdba74] text-sm font-semibold text-center">
                  {step.actionLabel}
                </Text>
                {step.actionHint ? (
                  <Text className="mt-1 text-center text-xs text-white/60">{step.actionHint}</Text>
                ) : null}
              </Pressable>
            ) : null}

            <View className="mt-5 flex-row items-center justify-between gap-3">
              <Pressable onPress={onSkip} className="flex-1 rounded-2xl bg-white/8 px-4 py-3">
